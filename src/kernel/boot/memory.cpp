...
extern "C" void* _preinit_pae_setup_paging()
{
    _preinit_write_serial(_preinit_serial_page_pae_msg);
    _preinit_pae_enabled = true;
    
    // Get the PDPT and PD entry values to use
    uint64 pdpte = (((uint32)&_preinit_pd) & 0xFFFFF000) | _preinit_pdpte_flags;
    uint64 pde = (((uint32)&_preinit_pt) & 0xFFFFF000) | _preinit_pde_flags;
